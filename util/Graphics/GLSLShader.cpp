...
static bool appears_nonempty(std::string str) {
	if (str == "No errors.\n") return false; //Oh, Intel, you crazy folks.
...
ProgramObjectRef get_program_object_from_text(string const &vert_text, string const &frag_text, AddVersion add_version) {
	vector< ShaderObjectRef > objects;
	std::string version = "";
	if (add_version == AddGLSL120) {
		version = "#version 120\n";
	}
	if (vert_text != "") {
		objects.push_back(get_shader_object_from_text(version + vert_text, GL_VERTEX_SHADER_ARB));
		if (objects.back().ref == NULL) {
			return ProgramObjectRef(NULL);
		}
	}
	if (frag_text != "") {
		objects.push_back(get_shader_object_from_text(version + frag_text, GL_FRAGMENT_SHADER_ARB));
		if (objects.back().ref == NULL) {
			return ProgramObjectRef(NULL);
		}
	}
